...
class Agent():
    def __init__(self, state_size, action_size, behavior_name, index_player, replay_memory_size=1e4, batch_size=512, gamma=0.99,
                 learning_rate = 1e4, target_tau=1e3, update_rate=100, seed=0): #affect your agent vs other agents
...
def transfer_6_action(n): #(front,side,rotational)
    list_actions = [(1,0,0), (2,0,0), (0,1,0), (0,2,0), (0,0,1), (0,0,2)]
    return list_actions[n]


def process_reward_list_striker(reward_list):
    # Reward for Scoring
    if reward_list[0] > 0:
        return 100
    # Reward for Opponent Score
...
try:
    for i_episode in range(1, num_episodes + 1):
        ...
        for i_game in range(num_game_each_episode):
            ...
            goal_reward_p1 = process_reward_list_striker(dec_p.reward)
            goal_reward_p2 = process_reward_list_striker(dec_p.reward) if two_striker == True else process_reward_list_defender(dec_p.reward)

            goal_reward_b1 = process_reward_list_striker(dec_b.reward)
            goal_reward_b2 = process_reward_list_striker(dec_b.reward) if two_striker == True else process_reward_list_defender(dec_b.reward)
            ...
            if(agent_b1.touch_ball_reward>0):
                agent_b1.upd_after_goal((min(45,i_game)))
            if(agent_b2.touch_ball_reward>0):
                agent_b2.upd_after_goal((min(45,i_game))) 
            '''
            if(goal_reward_b1>1):
                agent_b1.we_goll()
            if(goal_reward_b2<-1):
            ...
            reward_b1 =  agent_b1.touch_ball_reward + goal_reward_b1 + agent_b1.close_ball_reward + agent_b1.rew_for_ball_dist +agent_b1.rew_d_to_our_post
            reward_b2 =  agent_b2.touch_ball_reward + goal_reward_b2 + agent_b2.close_ball_reward + agent_b2.rew_for_ball_dist +agent_b2.rew_d_to_our_post

            # Reward for Purple Player
            reward_p1 =  agent_p1.touch_ball_reward + goal_reward_p1 + agent_p1.close_ball_reward + agent_p1.rew_for_ball_dist +agent_p1.rew_d_to_our_post
            reward_p2 =  agent_p2.touch_ball_reward + goal_reward_p2 + agent_p2.close_ball_reward + agent_p2.rew_for_ball_dist +agent_p2.rew_d_to_our_post
        ...
        if i_episode % 5 == 0:
            ...
            nn_last_p1_filename = './result/p1_act_backup' 
            nn_last_target_p1_filename = './result/p1_target_backup' 

            nn_last_p2_filename = './result/p2_act_backup'
            nn_last_target_p2_filename = './result/p2_target_backup'
            ...
            nn_last_b1_filename = './result/b1_act_backup'
            nn_last_target_b1_filename = './result/b1_target_backup'

            nn_last_b2_filename = './result/b2_act_backup'
            nn_last_target_b2_filename = './result/b2_target_backup'

            torch.save(agent_b1.network.state_dict(), nn_last_b1_filename)
            torch.save(agent_b2.network.state_dict(), nn_last_b2_filename)
            torch.save(agent_b1.target_network.state_dict(), nn_last_target_b1_filename)
            torch.save(agent_b2.target_network.state_dict(), nn_last_target_b2_filename)

except KeyboardInterrupt:
    print("INFO: KeyboardInterrupt has been caught. Saving Model")

    nn_last_p1_filename = './result/p1_act_'+your_name 
    nn_last_target_p1_filename = './result/p1_target_' +your_name 

    nn_last_p2_filename = './result/p2_act_'+your_name 
    nn_last_target_p2_filename = './result/p2_target_'+your_name 
    ...
    nn_last_b1_filename = './result/b1_act_'+your_name 
    nn_last_target_b1_filename = './result/b1_target_'+your_name 

    nn_last_b2_filename = './result/b2_act_'+your_name 
    nn_last_target_b2_filename = './result/b2_target_'+your_name 
